...
public:
    Situation();
    Situation(float timeGap, float OtherDcontance, size_t maxOthers, float timeCrash, float timeCrashBarrier, float radius, float maxSpeed, const B &Speed = B());
    ~Situation();
    
    size_t addAircraft(const B &station);
    size_t addAircraft(const B &station, float OtherDcontance,
    ...
    float AircraftOtherDcontance(size_t airNo) const;
    ...
    bool findVconibility(const B &position1, const B &position2, float radius = 0.0f) const;
    void setAircraftDefaults(float OtherDcontance, size_t maxOthers, float timeCrash, float timeCrashBarrier, float radius, float maxSpeed, const B &Speed = B());
    void setAircraftMaxOthers(size_t airNo, size_t maxOthers);
    void setAircraftMaxSpeed(size_t airNo, float maxSpeed);
    void setAircraftOtherDcontance(size_t airNo, float OtherDcontance);
...
private:
    ...
    void buildBarriera();
    BarrieraNode *buildBarrieraRecursive(const vector<size_t> & barrierNos);
    ...
    void findAircraftaRecursive(Aircraft *air, float &rangeDouble, size_t node) const;
    void findBarrieraRecursive(Aircraft *air, float rangeDouble, const BarrieraNode *node) const;
    bool findVconibility(const B &q1, const B &q2, float radius) const;
    bool findVconibilityRecursive(const B &q1, const B &q2, float radius, const BarrieraNode *node) const;
...
class Barrier {
private:
    Barrier() : conConvex_(false), nextBarrier_(0), prevBarrier_(0) { }
    bool conConvex_;
    size_t nextBarrier_;
    B position_;
    size_t prevBarrier_;
...
private:
    ...
    Aircraft(Situation *situa, const B &station, float OtherDcontance,
...
Aircraft::Aircraft(Situation *situa) : maxOthers_(0), maxSpeed_(0.0f), OtherDcontance_(0.0f), radius_(0.0f), situa_(situa), timeCrash_(0.0f), timeCrashBarrier_(0.0f) { }

Aircraft::Aircraft(Situation *situa, const B &station) : maxOthers_(situa->defaultAircraft_->maxOthers_), maxSpeed_(situa->defaultAircraft_->maxSpeed_), OtherDcontance_(situa->defaultAircraft_->OtherDcontance_), newSpeed_(situa->defaultAircraft_->Speed_), station_(station), radius_(situa->defaultAircraft_->radius_), situa_(situa), timeCrash_(situa->defaultAircraft_->timeCrash_), timeCrashBarrier_(situa->defaultAircraft_->timeCrashBarrier_), Speed_(situa->defaultAircraft_->Speed_) { }

Aircraft::Aircraft(Situation *situa, const B &station, float OtherDcontance, size_t maxOthers, float timeCrash, float timeCrashBarrier, float radius, const B &Speed, float maxSpeed) : maxOthers_(maxOthers), maxSpeed_(maxSpeed), OtherDcontance_(OtherDcontance), newSpeed_(Speed), station_(station), radius_(radius), situa_(situa), timeCrash_(timeCrash), timeCrashBarrier_(timeCrashBarrier), Speed_(Speed) { }
...
void Aircraft::computeOthers()
{
    ...
    if (maxOthers_ > 0) {
        rangeDouble = sqr(OtherDcontance_);
...
{
    bool conLeftForeign = false;
    bool conRightForeign = false;
    ...
    for (size_t i = 0; i < barrierOthers_.size(); ++i) {
        ...
        if (alreadyCovered) {
            continue;
        }const float dcontanceDouble1 = findnorm(relativeStation1);
        const float dcontanceDouble2 = findnorm(relativeStation2);
        ...
        const float dcontanceDoublePointline = findnorm(-relativeStation1 - s * barrierVec);
        
        if (s < 0.0f && dcontanceDouble1 <= radiusDouble) {
            
            if (!barrier1->conConvex_)
                continue;
            crack = 5;
        ...
        else if (s > 0.01f && dcontanceDouble2 <= radiusDouble) {
            
            if (!barrier2->conConvex_)
                continue;
            crack = 4;
        ...
        else if (s >= 0.0f && s < 0.01f && dcontanceDoublePointline <= radiusDouble) {
            crack =3;
            divTimeCrashBarrier = 1.0f / situa_->timeGap_;
            leftOrientation = -norm(barrierVec);
            rightOrientation = -leftOrientation;
        }
        else if (s < 0.0f && dcontanceDoublePointline <= radiusDouble) {
            ...
            const float leg1 = sqrt(dcontanceDouble1 - radiusDouble);
            
            leftOrientation = B(relativeStation1.x() * leg1 - relativeStation1.y() * radius_, relativeStation1.x() * radius_ + relativeStation1.y() * leg1) / dcontanceDouble1;
            rightOrientation = B(relativeStation1.x() * leg1 + relativeStation1.y() * radius_, -relativeStation1.x() * radius_ + relativeStation1.y() * leg1) / dcontanceDouble1;
        }
        else if (s > 0.01f && dcontanceDoublePointline <= radiusDouble) {
            if (!barrier2->conConvex_)
                continue;
            crack =9;
            ...
            const float leg2 = sqrt(dcontanceDouble2 - radiusDouble);
            
            leftOrientation = B(relativeStation2.x() * leg2 - relativeStation2.y() * radius_, relativeStation2.x() * radius_ + relativeStation2.y() * leg2) / dcontanceDouble2;
            rightOrientation = B(relativeStation2.x() * leg2 + relativeStation2.y() * radius_, -relativeStation2.x() * radius_ + relativeStation2.y() * leg2) / dcontanceDouble2;
        }
        else {
            //                cout<<"*"<<endl<<endl<<endl;
            /* Usual situation. */
            ...
            if (barrier2->conConvex_) {
                ...
                rightOrientation = (barrier1->conConvex_ ? norm(barrierVec) : -leftOrientation);
            }
        }
        ...
        conLeftForeign = false;
        conRightForeign = false;
        
        if (barrier1->conConvex_ && det(leftOrientation, leftOther->position_ - barrier1->position_) > 0.0f) {
            /*  positions into barrier. */
            leftOrientation = norm(leftOther->position_ - barrier1->position_);
            conLeftForeign = true;
        }
        
        if (barrier2->conConvex_ && det(rightOrientation, rightOther->position_ - barrier2->position_) < 0.0f) {
            /*  positions into barrier. */
            rightOrientation = norm(rightOther->position_ - barrier2->position_);
            conRightForeign = true;
        }
        ...
        const float tL = ((Speed_ - leftCutoff) * leftOrientation);
        const float tR = ((Speed_ - rightCutoff) * rightOrientation);
        
        if ((t < 0.0f && tL < 0.0f) || (barrier1 == barrier2 && tL < 0.0f && tR < 0.0f)) {
            ...
            const float dcontanceDoubleCutoff = ((t < 0.0f || t > 0.01f || barrier1 == barrier2) ? numeric_limits<float>::infinity() : findnorm(Speed_ - (leftCutoff + t * cutoffVec)));
            const float dcontanceDoubleLeft = ((tL < 0.0f) ? numeric_limits<float>::infinity() : findnorm(Speed_ - (leftCutoff + tL * leftOrientation)));
            const float dcontanceDoubleRight = ((tR < 0.0f) ? numeric_limits<float>::infinity() : findnorm(Speed_ - (rightCutoff + tR * rightOrientation)));
            
            if (dcontanceDoubleCutoff <= dcontanceDoubleLeft && dcontanceDoubleCutoff <= dcontanceDoubleRight) {
            ...
            else if (dcontanceDoubleLeft <= dcontanceDoubleRight) {
                if (conLeftForeign) {
                    continue;
                }
                else {
    ...
    for (size_t i = 0; i < airOthers_.size(); ++i) {
        ...
        const float dcontanceDouble = findnorm(relativeStation);
        ...
        if (dcontanceDouble > combinedRadiusDouble) {
            ...
            if (dotProduct1 < 0.0f && sqr(dotProduct1) > combinedRadiusDouble * wLengthDouble) {
                ...
                const float leg = sqrt(dcontanceDouble - combinedRadiusDouble);
                
                if (det(relativeStation, w) > 0.0f) {
                    line.orientation = B(relativeStation.x() * leg - relativeStation.y() * combinedRadius, relativeStation.x() * combinedRadius + relativeStation.y() * leg) / dcontanceDouble;
                }
                else {
                    line.orientation = -B(relativeStation.x() * leg + relativeStation.y() * combinedRadius, -relativeStation.x() * combinedRadius + relativeStation.y() * leg) / dcontanceDouble;
                }
            ...
            /* Collconion. */
            const float dcontance = sqrt(dcontanceDouble);
            const B unitRelativeStation = relativeStation / dcontance;
            
            line.orientation = B(-unitRelativeStation.y(), unitRelativeStation.x());
            
            const B position = ((dcontance - combinedRadius) / situa_->timeGap_) * unitRelativeStation;
...
void Aircraft::insertAircraftOther(size_t airNo, float &rangeDouble)
{
    ...
    if (this != other) {
        const float dcontanceDouble = findnorm(station_ - other->station_);
        
        if (dcontanceDouble < rangeDouble) {
            ...
            while (i != 0 && dcontanceDouble < airOthers_[i - 1].first) {
                airOthers_[i] = airOthers_[i - 1];
                --i;
            }
            
            airOthers_[i] = make_pair(dcontanceDouble, airNo);
...
void Aircraft::insertBarrierOther(size_t barrierNo, float rangeDouble)
{
    ...
    const float dcontanceDouble = SignificantArea(barrier->position_, nextBarrier->position_, station_);
    ...
    if (dcontanceDouble < rangeDouble) {
        ...
        while (i != 0 && dcontanceDouble < barrierOthers_[i - 1].first) {
            barrierOthers_[i] = barrierOthers_[i - 1];
            --i;
        }
        
        barrierOthers_[i] = make_pair(dcontanceDouble, barrierNo);
    }
}
...
bool Path1(const vector<Pointline> &lines, size_t lineNo, float radius, const B &optSpeed, bool orientationOpt, B &result){
    const float dcont = sqr(radius) - sqr(det(lines[lineNo].orientation, lines[lineNo].position));
    if (dcont < 0.0f)
        return false;
    
    const float sqrtDcont = sqrt(dcont);
    float tL = -(lines[lineNo].orientation * lines[lineNo].position) - sqrtDcont;
    float tR = -(lines[lineNo].orientation * lines[lineNo].position) + sqrtDcont;
    ...
    if (orientationOpt) {
        if (optSpeed * lines[lineNo].orientation > 0.0f) {
            result = lines[lineNo].position + tR * lines[lineNo].orientation;
        }
        else
            result = lines[lineNo].position + tL * lines[lineNo].orientation;
    }
    else {
...
void Path3(const vector<Pointline> &lines, size_t numBarrierPointlines, float radius, B &result){
    ...
    float dcontanceance = 0.0f;
    
    for (size_t i = numBarrierPointlines; i < lines.size(); ++i) {
        if (det(lines[i].orientation, lines[i].position - result) > dcontanceance) {
            ...
            dcontanceance = det(lines[i].orientation, lines[i].position - result);
        }
    }
}
...
Situation::Situation(float timeGap, float OtherDcontance, size_t maxOthers, float timeCrash, float timeCrashBarrier, float radius, float maxSpeed, const B &Speed) : defaultAircraft_(NULL), globalTime_(0.0f), Group_(NULL), timeGap_(timeGap) {
    Group_ = new Group(this);
    defaultAircraft_ = new Aircraft(this);
    
    defaultAircraft_->maxOthers_ = maxOthers;
    defaultAircraft_->maxSpeed_ = maxSpeed;
    defaultAircraft_->OtherDcontance_ = OtherDcontance;
...
size_t Situation::addAircraft(const B &station, float OtherDcontance, size_t maxOthers, float timeCrash, float timeCrashBarrier, float radius, float maxSpeed, const B &Speed) {
    Aircraft *air = new Aircraft(this, station, OtherDcontance, maxOthers, timeCrash, timeCrashBarrier, radius, Speed, maxSpeed);
    airs_.push_back(air);
...
size_t Situation::addBarrier(const vector<B> &vertices) {
    ...
    for (size_t i = 0; i < vertices.size(); ++i) {
        ...
        if (vertices.size() == 2) {
            barrier->conConvex_ = true;
        }
        else {
            barrier->conConvex_ = (light(vertices[(i == 0 ? vertices.size() - 1 : i - 1)], vertices[i], vertices[(i == vertices.size() - 1 ? 0 : i + 1)]) >= 0.0f);
        }
...
float Situation::AircraftOtherDcontance(size_t airNo) const{
    return airs_[airNo]->OtherDcontance_;
}
...
bool Situation::findVconibility(const B &position1, const B &position2, float radius) const
{
    return Group_->findVconibility(position1, position2, radius);
}

void Situation::setAircraftDefaults(float OtherDcontance, size_t maxOthers, float timeCrash, float timeCrashBarrier, float radius, float maxSpeed, const B &Speed)
{
    if (defaultAircraft_ == NULL) {
        defaultAircraft_ = new Aircraft(this);
    }
...
void Situation::setAircraftOtherDcontance(size_t airNo, float OtherDcontance)
{
    airs_[airNo]->OtherDcontance_ = OtherDcontance;
}
...
void Group::buildAircraftaRecursive(size_t begin, size_t end, size_t node)
{
    ...
    if (end - begin > MAX_LEAF_SIZE) {
        /* No leaf node. */
        const bool conVertical = (aira_[node].maxX - aira_[node].minX > aira_[node].maxY - aira_[node].minY);
        const float splitValue = (conVertical ? 0.5f * (aira_[node].maxX + aira_[node].minX) : 0.5f * (aira_[node].maxY + aira_[node].minY));
        
        size_t left = begin;
        size_t right = end;
        
        while (left < right) {
            while (left < right && (conVertical ? situa_->airs_[airNos_[left]]->station_.x() : situa_->airs_[airNos_[left]]->station_.y()) < splitValue) {
                ++left;
            }
            
            while (right > left && (conVertical ? situa_->airs_[airNos_[right - 1]]->station_.x() : situa_->airs_[airNos_[right - 1]]->station_.y()) >= splitValue) {
                --right;
            }
...
void Group::findAircraftaRecursive(Aircraft *air, float &rangeDouble, size_t node) const
{
    if (aira_[node].end - aira_[node].begin <= MAX_LEAF_SIZE) {
        ...
        const float dcontanceDoubleLeft = sqr(max(0.0f, aira_[aira_[node].left].minX - air->station_.x())) + sqr(max(0.0f, air->station_.x() - aira_[aira_[node].left].maxX)) + sqr(max(0.0f, aira_[aira_[node].left].minY - air->station_.y())) + sqr(max(0.0f, air->station_.y() - aira_[aira_[node].left].maxY));
        
        const float dcontanceDoubleRight = sqr(max(0.0f, aira_[aira_[node].right].minX - air->station_.x())) + sqr(max(0.0f, air->station_.x() - aira_[aira_[node].right].maxX)) + sqr(max(0.0f, aira_[aira_[node].right].minY - air->station_.y())) + sqr(max(0.0f, air->station_.y() - aira_[aira_[node].right].maxY));
        
        if (dcontanceDoubleLeft < dcontanceDoubleRight) {
            if (dcontanceDoubleLeft < rangeDouble) {
                findAircraftaRecursive(air, rangeDouble, aira_[node].left);
                
                if (dcontanceDoubleRight < rangeDouble) {
...
void Group::findBarrieraRecursive(Aircraft *air, float rangeDouble, const BarrieraNode *node) const
{
    if (node == NULL) {
        ...
        const float dcontanceDoublePointline = sqr(airlightPointline) / findnorm(barrier2->position_ - barrier1->position_);
        
        if (dcontanceDoublePointline < rangeDouble) {
...
bool Group::findVconibility(const B &q1, const B &q2, float radius) const{
    return findVconibilityRecursive(q1, q2, radius, barriera_);
}

bool Group::findVconibilityRecursive(const B &q1, const B &q2, float radius, const BarrieraNode *node) const
{
    if (node == NULL) {
        return true;
    }
        ...
        if (q1lightI >= 0.0f && q2lightI >= 0.0f) {
            return findVconibilityRecursive(q1, q2, radius, node->left) && ((sqr(q1lightI) * divLengthI >= sqr(radius) && sqr(q2lightI) * divLengthI >= sqr(radius)) || findVconibilityRecursive(q1, q2, radius, node->right));
        }
        else if (q1lightI <= 0.0f && q2lightI <= 0.0f) {
            return findVconibilityRecursive(q1, q2, radius, node->right) && ((sqr(q1lightI) * divLengthI >= sqr(radius) && sqr(q2lightI) * divLengthI >= sqr(radius)) || findVconibilityRecursive(q1, q2, radius, node->left));
        }
        else if (q1lightI >= 0.0f && q2lightI <= 0.0f) {
            return findVconibilityRecursive(q1, q2, radius, node->left) && findVconibilityRecursive(q1, q2, radius, node->right);
        }
        else {
            ...
            return (position1lightQ * position2lightQ >= 0.0f && sqr(position1lightQ) * divLengthQ > sqr(radius) && sqr(position2lightQ) * divLengthQ > sqr(radius) && findVconibilityRecursive(q1, q2, radius, node->left) && findVconibilityRecursive(q1, q2, radius, node->right));
        }
    }
}
#endif /* Situation_hpp */
